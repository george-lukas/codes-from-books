...
(defstruct varq name (binding +unbound+))

(defun bound-p (var) (not (eql (binding-var var) +unbound+)))

(defmacro derefq (exp)
  "Follow pointers for bound variables."
  `(progn (loop :while (and (variable-p ,exp) (bound-p ,exp))
	        :do (setf ,exp (binding-var ,exp)))
	  ,exp))

(defun set-binding! (var value)
  "Set var's binding to value.  Always succeeds (returns t)."
  (setf (binding-var var) value) t)

(defun unify! (x y)
  "Follow pointers for bound variables."
  (cond ((eql (derefq x) (derefq y)) t)
...
(defun undo-bindings! (old-trail)
  ...
  (loop :until (= (fill-pointer *trail*) old-trail)
        :do (setf (binding-var (vector-pop *trail*)) +unbound+)))

(defvar *var-counter* 0)

(defstruct (varq (:constructor ? ())
...
(defun follow-binding (var bindings)
  ...
  (let ((b (get-binding var bindings)))
    (if (eql (first b) (rest b)) b
        (or (follow-binding (rest b) bindings)
            b))))
        ...
        ((variable-p arg)
         (let ((binding (get-binding arg bindings)))
           (if (and (not (null binding))
                    (not (eql arg (binding-val binding))))
...
(defun compile-unify-variable (x y bindings)
  ...
  (let* ((xb (follow-binding x bindings))
         ...
         (y1 (if yb (rest yb) y)))
    (cond                                                 ; Case:
      ((or (eql x '?) (eql y '?)) (values t bindings))      ; 12
      ((not (and (eql x x1) (eql y y1)))              ; deref
       (compile-unify x1 y1 bindings))
      ((find-if-anywhere x1 y1) (values nil bindings))    ; 11
      ((consp y1)                                         ; 7,10
...
(defun compile-body (body cont bindings)
  ...
  (cond ((null body)
	...
	(t (let* ((goal (first body))
		  ...
		  (macro-val (if macro (funcall macro goal (rest body) 
						cont bindings))))
	     (if (and macro (not (eql macro-val :pass))) macro-val
		 `(,(make-predicate (predicate goal)
				    (relation-arity goal))
		    ,@(mapcar #'(lambda (arg)
				  (compile-arg arg bindings))
			      (args goal))
		    ,(if (null (rest body)) cont
			 `#'(lambda ()
			      ,(compile-body 
				(rest body) cont
