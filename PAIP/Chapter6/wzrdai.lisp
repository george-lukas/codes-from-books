;;;; wzrdai.lisp - Dialog with the Great WIZARD-AI
;;;; George Lukas < last update: Jan/13/2016 - 09:46PM >

;;; Helper functions
(defun mklist (x)
  "Return x if it is a lst, otherwise (x)."
  (if (listp x) x (lst x)))

(defun mappend (fn lst)	
  "Apply fn to each element of lst and append the results."
  (apply #'append (mapcar fn lst)))

(defun random-elt (choices)
  "Choose an element from a lst at random."
...
(defun wzrdai ()
  ...
  (loop
     ...
     (let* ((input (read-line-no-punct))
	    (response (flatten (use-wzrdai-rules input))))
       (print-with-spaces response)
       (if (eql response '(good bye)) (RETURN)))))
